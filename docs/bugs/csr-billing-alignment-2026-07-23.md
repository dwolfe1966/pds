...
**Deploy candidate:** `admin.40bd8cf3.js` (NOT yet deployed).
...
rest is vocabulary depth (the CSR's finer M-codes sitting beneath the admin view's 5 coarse terminals).
...
flags raised on the fraud/stolen cohort. Deploy `admin.40bd8cf3.js` to make it live.
...
The CSR uses a finer M-code vocabulary (`trial-M0-paid`, `trial-D1.1`, `subscriber-M1-paid`, …) that sits
...
## 3. Changes made (this session, in `admin.40bd8cf3.js`)
...
|---|---|---|---|
| `trial-M0-paid`, `trial-M0-unpaid`, `subscriber-M{n}-paid` (status active) | `[active]` | Yes | healthy |
| `trial-D{n}.{r}` / dunning (status active + retry) | `[active]` | Yes (at-risk) | **+ ⚠ flag if the decline was fraud/stolen** |
| `trial-M0-norenewal` / `subscriber-M{n}-norenewal` (cancel-at-period-end) | `[canceled (pending period end)]` | Yes | access until period end |
...
After the §3 changes, the CSR access bucket reflects the admin view for every category. The M-code adds *detail beneath*
...
|---|---|---|---|
| `[active]` | `trial-M0-paid` | 18 | ✅ |
| `[active]` | `trial-D1.2` (dunning, retry 2) | 16 | ✅ |
| `[active]` | `trial-D1.1` (dunning, retry 1) | 13 | ✅ |
| `[active]` | `trial-M0-unpaid` (validate-only / cascade) | 8 | ✅ |
| `[canceled (pending period end)]` | `trial-M0-norenewal` | 7 | ✅ |
...
Every admin terminal maps to a consistent CSR state; our M-code adds detail *beneath* the admin terminal without
...
## 6. Open items
- **Deploy `admin.40bd8cf3.js`** (the alignment isn't live until then).

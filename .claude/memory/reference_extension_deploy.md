...
- **Host scope (2026-08-10):** CWS flagged "Broad Host Permissions" on submit. FIXED — content script now scoped to a fixed ~30-site list (23 broker domains from recipes.js + 7 social nets) instead of `<all_urls>` (commit b4d29e6). Keeps autofill/detection/discovery + broker/social flags; manifest matches are install-granted so no per-site friction; only loss = generic account-form tip on arbitrary sites. Adding a broker later = manifest update + version bump.
- **Owner-only steps:** register CWS dev account ($5 one-time) + identity/domain verify, upload zip, paste listing, complete privacy form, set Unlisted, submit. Optional `history` is still "sensitive" (review), but broad-host is resolved.

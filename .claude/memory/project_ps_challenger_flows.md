...
- **C SRP self-treatment (built):** `SelfExposureIntro` banner renders above the teaser when `getVariant()==='self'` — reframes the SRP as the searcher's OWN exposure + teases the standing service (claim + who's-searching alerts). HONEST: no fabricated "N people searched you" count (no real pre-pay WSFY data for a non-member) — stated as a capability. Real records still render below in the standard teaser.

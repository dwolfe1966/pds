...
- [Org dynamic: NIC team](project_org_dynamic_nic_team.md) — David=CEO new co; Jerome/Kwan (old CEO/CTO) frame him as "marketer/FED", critique "sloppiness"; Tim conflicted; artifacts to Tim must be immaculate; lead with backend evidence + outcomes
...
- [Cancel: online only for CA + NYC](project_cancel_ca_nyc_online.md) — ZIP-driven; unknown ZIP now FAILS CLOSED → CSR (2026-09-02); reason+save modal universal; `subscription_cancel`+`cancel_redirect_cs` must be summed

...
- [WSFY self-build](project_wsfy_self_build.md) — build Who's-Searching-For-You ourselves (BC has none); Phase 1 ingest + Phase 2 tiered reveal (free tease / paid detail, server-side masking) SHIPPED (bundle public.03ee67a5.js, not yet on BC); open: auth-harden (tier client-asserted) + opt-out suppression

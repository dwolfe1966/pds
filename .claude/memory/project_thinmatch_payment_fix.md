...
**Advisor guards applied**: import `generatePassword` (silent-ReferenceError footgun avoided); set `_pwAuto` (else user locked out of paid account); keep signed-in branches; keep `/payment` no-personId. Build clean. **NEXT: owner uploads public.7e4ba4bb.js to BC; verify v11 thin-match → Continue → payment live.**

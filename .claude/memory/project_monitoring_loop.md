...
**Honesty rule (same as cron):** detection SUSPECTS, never asserts. The endpoint only acts when there's an ACTIVE removal node for that source, and it logs a member-confirmable `reappearance_suspected` identity_event (dedup per episode via node.last_changed date) — it does NOT auto-flip control_status to `reappeared`. host→source_key via SLD (spokeo.com→spokeo); backend validates against source_registry so a bad guess no-ops. New: `getNodeBySource()`, `/api/exposure-detection`, bell icon 👀. Committed c179c78 + pushed.

**Removal-completion detection (the inverse signal):** same extension engine, `signal:'absent'`. When the member searches themselves on a managed broker and the listing is NOT found, content.js reports absent WITH a search-context proof (`searchedForMember`: name in URL/title, name in a search input, or an explicit "no results" phrase) — absence without that proof is ignored (a homepage visit isn't "removed"). Backend emits `removal_verified_suspected` only for nodes still in optout_requested/reappeared. Member confirms → `confirmRemoved(sourceKey)` → control_status='removed'. Deliberately member-confirmable, NOT silent auto-advance: a false "you're removed!" (false reassurance) is worse than a false "check again." Bell icon ✅; green prompt in the DF tracker.

**Suspected signals surfaced in DF too (not just the bell):** DigitalFootprint fetches identity_events, filters the two suspected types, and renders an inline prompt on the affected removal-tracker row (amber "is it back?" for reappearance, green "mark removed?" for completion). Shares the `idlNotifHandled` localStorage set with the bell so resolving in one place clears the other.

**Confirmation flip (wired, consumer-only):** the NotificationBell renders "Yes, I'm listed" / "Not there" on `reappearance_suspected` events. "Yes" → `confirmReappearance(sourceKey)` in memberEnrichment → `setExposureControl(controlStatus:'reappeared', controlMethod:'member_confirmed')` (reuses the existing exposure route; no backend change). `confirmReappearance` is the ONLY authorized writer of `reappeared` — the member's click is what turns a suspicion into a fact. Handled event ids tracked client-side in localStorage `idlNotifHandled` (append-only events have no server "resolve"; the real state change persists server-side on the node, so the DF removal tracker then shows "Re-appeared / Re-submit now"). Full loop closed: cron reminds → extension suspects → member confirms → reappeared → re-submit.

...
**Abandon cron wiring:** account exists → mint auto-login (redirect `/auth/session?next=<report/dashboard/my-identity>`); NO account (abandoner never completed signup, e.g. email-capture-only) → fall back to prefilled `/name/landing/v3?fn=…` resume link (already built).

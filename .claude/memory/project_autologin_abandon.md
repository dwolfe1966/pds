...
**Do-not-contact block (owner 2026-07-28):** NO email ever sent to `bytecrtrs.com` or any subdomain — `isBlockedRecipient()` guard inside `sendEmail` (the sole provider dispatch point, so ALL campaigns covered) + upstream filters in cron/preview to avoid retry churn. Add more via `EMAIL_BLOCKED_DOMAINS` (comma-sep). Case-insensitive; allows look-alikes (notbytecrtrs.com).

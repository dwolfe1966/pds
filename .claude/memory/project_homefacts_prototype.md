...
**Next data modules (free/public):** FEMA NRI (disasters) — ⚠️ static CSV download 403s + guessed ArcGIS FeatureServer 400s; must locate the ArcGIS-Hub NRI county layer (validate Travis 48453 before 2k sweep); join city→county via **FCC block API** (verified: lat/lng→county FIPS). Then NCES (schools), FBI CDE (crime, api.data.gov key), EPA EJScreen/ECHO (environment).

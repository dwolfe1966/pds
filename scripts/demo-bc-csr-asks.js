 ...
 * It asserts nothing — it calls the methods and prints what BC returns. For each ask:
 *   WE CALL → REQUEST (exact bytes on the wire) → BC RETURNS → VERDICT   (+ contrast).
 * Read-only: only finders/getters are called. No writes, sales, or mutations.
 *
 * Run:      node scripts/demo-bc-csr-asks.js            (uses a shared DEV test account)
 *           CSR_USER='…' CSR_PWD='…' node scripts/demo-bc-csr-asks.js   (your own account)
 *           VERBOSE=1 node scripts/demo-bc-csr-asks.js  (also dumps raw request + response)
 * Env:      CSR_USER, CSR_PWD · ADMIN_HOST · CONSUMER_URL · TEST_USER_ID · VERBOSE
 * ─────────────────────────────────────────────────────────────────────────── */
...
(async () => {
  ...
  // Capture the exact request (method + path + body) the page sends, tagged by `phase`.
  let phase = 'login';
  let loginQs = '';
  const sent = {};
  page.on('request', (req) => {
    ...
    if (/\/(database\/search|commerce\/offer\/findByShmName|commerceBilling\/sale|contactMessage\/admin\/find)/.test(u)) {
      let b = null; try { b = req.postData(); } catch {}
      (sent[phase] = sent[phase] || []).push({ method: req.method(), path: u.replace(ADMIN, '').split('?')[0], body: b });
    }
  });

  // login + confirm (retry the occasional cold-session 403)
  ...
  // direct /database/search call → returns {status, count, msg, json}
  const direct = async (body) => page.evaluate(async ({ body, qs }) => {
    try { const res = await fetch(`/api/database/search${qs}`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) });
      const j = await res.json().catch(()=>null); return { status: res.status, count: Array.isArray(j?.docs)?j.docs.length:null, msg: j?.message||null, json: j }; }
    catch (e) { return { thrown: e.message }; }
  }, { body, qs: loginQs });

  const showReq = (ph) => { const rs = sent[ph] || []; if (!rs.length) { line('  REQUEST :  (local namespace check — no network request)'); return; } rs.forEach(r => line(`  REQUEST :  ${r.method} ${r.path}${r.body ? '  ' + r.body : ''}`)); };
  const raw = (label, obj) => { if (VERBOSE) line(`  RAW ${label}: ${typeof obj === 'string' ? obj : JSON.stringify(obj)}`); };

  line(); line('████  BC CSR API — live demonstration  ████');
  line(`account: ${USER}   env: ${ADMIN}${VERBOSE ? '   [VERBOSE]' : ''}`);
  ...
  line(`  WE CALL :  POST /commerce/offer/findByShmName { shmName:'comp.offer.signup.main' }   (CSR session, 3 brand variants)`);
  showReq('A');
  line(`  BC RETURNS: no-brand → ${aNo.status} "${aNo.msg}"   idlookup → ${aId.status} "${aId.msg}"   bytecrtrs → ${aBc.status} "${aBc.msg}"`);
  raw('(no-brand response)', aNo.json);
  line(`  CONTRAST:  SAME shmName @ consumer (${CONSUMER}) → hasPrice=${aConsumer&&aConsumer.hasPrice} s0=${JSON.stringify(aConsumer&&aConsumer.s0)}`);
  ...
  line(`  WE CALL :  POST /database/search { collectionName:'commerceOrder' }   (3 brand variants)`);
  showReq('C');
  line(`  BC RETURNS: no-brand → ${cNo.status} "${cNo.msg}"   idlookup → ${cId.status} "${cId.msg}"   bytecrtrs → ${cBc.status} "${cBc.msg}"`);
  raw('(no-brand response)', cNo.json);
  line(`  VERDICT :  403 "Invalid Database Search Role" for every brand → no global order search.`);

  // ── CONFIRM — userContact data model ───────────────────────────────
  phase = 'Q';
  const fc = await page.evaluate(async ({ uid }) => { try { const c = window.CsrWrapper.getInstance({ endpointUrl:'/api' }); const r = await c.api.user.findUserContacts.call(c.api.user, { userId: uid }); const d=r?.getData?r.getData():r; return { count:(d?.docs||[]).length, json: d }; } catch(e){ return { thrown:e.message }; } }, { uid: TEST_USER_ID });
  ...
  line(`  WE CALL :  csrWrapper.api.user.findUserContacts({ userId }) ; and direct {collectionName:'userContact'}`);
  showReq('Q');
  line(`  BC RETURNS: findUserContacts → ${fc.count} contactMessage docs ; userContact direct → ${uc.status} "${uc.msg}"`);
  raw('(findUserContacts response)', fc.json);
  ...
  if (!VERBOSE) line('  (run with VERBOSE=1 for the raw request bodies + full responses)');
  hr();
  await browser.close();

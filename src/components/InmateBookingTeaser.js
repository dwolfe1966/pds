      ...
      <div style={{ display: 'flex', gap: 8, overflow: 'hidden' }}>
        {records.map((r, i) => (
          <div key={i} style={{ flex: '0 0 auto', width: 68, textAlign: 'center' }}>
            <div style={{ width: 68, height: 68, borderRadius: 8, background: '#dbe9f2', overflow: 'hidden', position: 'relative' }}>
              {r.mugshotUrl
                ? <img src={r.mugshotUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'blur(5px)' }} />
                : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, filter: 'blur(2px)' }}>👤</div>}
            </div>

...
const UserDetailPage = () => {
  ...
  // Current CSR — for audit attribution on the impersonation action (who minted the link). Available to
  // ALL CSRs (owner 2026-07-21); the admin app is already staff-gated and each use is audit-noted.
  const { user: csrUser } = useAuth();
  ...
  const [impersonateUrl, setImpersonateUrl] = useState(''); // minted login-as-user link (ephemeral, never persisted)
  ...
  // ── Impersonate: mint a one-click "log in as this customer" link (bearer credential) ──
  // Opens the BC loginLink so the CSR can troubleshoot in the customer's own session. The URL is a
  // live account-takeover credential, so: admins-only (gate above), an audit note is written that
  // NEVER stores the URL, and we open it in a fresh tab (noopener). NOTE: full impersonation isn't
  // complete until the consumer session-adoption route lands — until then this is the dev-test vehicle
  // to observe what BC's loginLink does (see docs/BC_AUTOLOGIN_ASK.md).
  const handleImpersonate = async () => {
    if (!window.confirm(
      `Generate a one-click link to log in AS ${getFullName(user)} (${user?.email || 'this customer'}) and troubleshoot their account?\n\n`
      + `This creates a link that signs you in as this customer. Your action is logged.`
    )) return;
    ...
    try {
      ...
      // Audit note — record WHO/WHEN, never the URL itself. Fire-and-forget (its Timeline refresh is
      // what produced the benign contactMessage 404 / userContact 403 noise; don't block the CSR).
      api.adminCreateNote({
        userId: id,
        message: `[ACTION] Impersonation login link generated by ${csrUser?.email || 'CSR'} to troubleshoot this account.`,
        contentType: 'text/plain',
      }).catch(() => { /* best-effort */ });
    } catch (err) {
      showToast(`Could not generate login link: ${err?.message || 'failed'}`, 'error');
    } finally {
  ...
  return (
    <main className={styles.page}>
      ...
      {/* Impersonation link modal — visible confirmation + explicit Open/Copy (no popup-blocker). The URL
          is a live login-as-user credential held only here in state; closing the modal drops it. */}
      {impersonateUrl && (
        <div
          role="dialog"
          aria-modal="true"
          onClick={() => setImpersonateUrl('')}
          style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(17,24,39,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.25rem' }}
        >
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 460, background: '#fff', borderRadius: 14, padding: '1.5rem', boxShadow: '0 10px 40px rgba(0,0,0,0.25)' }}>
            <h3 style={{ margin: '0 0 0.4rem', fontSize: '1.1rem', fontWeight: 800, color: '#111827' }}>Log in as {getFullName(user)}</h3>
            <p style={{ margin: '0 0 0.9rem', fontSize: '0.85rem', color: '#4b5563' }}>
              One-time troubleshooting link — opens their session in a new tab. Do not share it; it signs the opener in as this customer.
            </p>
            <input
              readOnly
              value={impersonateUrl}
              onFocus={(e) => e.target.select()}
              style={{ width: '100%', boxSizing: 'border-box', padding: '0.6rem 0.7rem', fontSize: '0.8rem', border: '1px solid #d1d5db', borderRadius: 8, color: '#374151', background: '#f9fafb' }}
            />
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button
                type="button"
                onClick={() => { window.open(impersonateUrl, '_blank', 'noopener,noreferrer'); }}
                style={{ flex: 1, padding: '0.7rem', fontSize: '0.9rem', fontWeight: 700, color: '#fff', background: '#4338ca', border: 'none', borderRadius: 8, cursor: 'pointer' }}
              >
                Open in new tab
              </button>
              <button
                type="button"
                onClick={() => { navigator.clipboard.writeText(impersonateUrl).then(() => showToast('Login link copied.', 'success')).catch(() => {}); }}
                style={{ padding: '0.7rem 1rem', fontSize: '0.9rem', fontWeight: 700, color: '#4338ca', background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: 8, cursor: 'pointer' }}
              >
                Copy
              </button>
              <button
                type="button"
                onClick={() => setImpersonateUrl('')}
                style={{ padding: '0.7rem 1rem', fontSize: '0.9rem', fontWeight: 600, color: '#6b7280', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, cursor: 'pointer' }}
              >
                Close
              </button>
            </div>

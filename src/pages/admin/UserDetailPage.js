...
const UserDetailPage = () => {
  ...
  // Current CSR (for the impersonation gate + audit attribution). Impersonation is full account
  // takeover — gated tighter than suspend: admins only (any csr can suspend).
  const { user: csrUser } = useAuth();
  const isAdminRole = csrUser?.role === 'admin';
  ...
  const handleImpersonate = async () => {
    ...
    try {
      ...
      // Audit note — record WHO/WHEN, never the URL itself (it's a working takeover link).
      try {
      ...
      } catch { /* note is best-effort; don't block the CSR */ }
      window.open(url, '_blank', 'noopener,noreferrer');
      showToast('Login-as-user link opened in a new tab.', 'success');
    } catch (err) {
    ...
    <main className={styles.page}>
      ...
      <div className={styles.layout}>
        ...
        <aside className={styles.profileCard}>
          ...
          {isAdminRole && (
            <button
              title="Generate a one-click link to log in as this customer and troubleshoot their account. Admins only; the action is logged."

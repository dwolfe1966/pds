...
const UserDetailPage = () => {
  ...
  const handleImpersonate = async () => {
    ...
    // Open the tab SYNCHRONOUSLY (inside the click gesture) so it isn't popup-blocked — the URL is
    // minted async, so we navigate this blank tab once it resolves. (noopener via rel on navigation.)
    ...
    try {
      ...
      if (win) { win.opener = null; win.location.href = url; }
        ...
        // Popup was blocked — copy the link so the CSR can paste it. (Bearer credential; not stored.)
        try { await navigator.clipboard.writeText(url); } catch { /* clipboard may be unavailable */ }
        showToast('Popup blocked — login link copied to clipboard. Paste into a new tab.', 'info');
      }
      // Audit note — record WHO/WHEN, never the URL itself (it's a working takeover link). Fire-and-forget
      // so it never delays opening the tab (and its Timeline refresh doesn't block the CSR).
      api.adminCreateNote({
        userId: id,
        message: `[ACTION] Impersonation login link generated by ${csrUser?.email || 'CSR'} to troubleshoot this account.`,
        contentType: 'text/plain',
      }).catch(() => { /* best-effort */ });
      if (win) showToast('Login-as-user link opened in a new tab.', 'success');
    } catch (err) {
      if (win) win.close();

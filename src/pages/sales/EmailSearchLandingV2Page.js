...
/**
 * Email search landing v2 – Generic Email Lookup.
 * 3-step flow: Email → Context/Location → Confirm → Results.
 */
const EmailSearchLandingV2Page = () => {
  const brand = useBrand();
  useLandingTrack('email', 'v2');
  ...
  useEffect(() => {
    ...
    if (step === 'searching-one') {
      timer = setTimeout(() => { track('search_step', { step: 'context', search_type: 'email', variant: 'v2' }); setStep('context'); }, 1700);
    }
    return () => {
  ...
  useEffect(() => {
    ...
    const runSearch = async () => {
      try {
        setFinalStatus('Searching email owner records...');
        setFinalProgress(10);
  ...
  const startSearch = (e) => {
    ...
    track('search_step', { step: 'searching-one', search_type: 'email', variant: 'v2' });
    setStep('searching-one');
  };

  const continueFromContext = () => { track('search_step', { step: 'confirm', search_type: 'email', variant: 'v2' }); setStep('confirm'); };
  ...
  const handleConfirm = () => {
    ...
    track('search_step', { step: 'final-search', search_type: 'email', variant: 'v2' });
    track('fcra_agree', { search_type: 'email', variant: 'v2' });
    ...
    <main className={styles.main}>
      <div className={styles.wrapper}>
        <p className={styles.pageSubtitle}>
          <span className={styles.pageSubtitleIcon} aria-hidden>✉️</span> Email Lookup
        </p>
        ...
        <div className={styles.card}>
          <h1 className={styles.title}>Who Owns This Email?</h1>
          <p className={styles.subtitle}>
            Enter any email address to search 12 billion public records and find the owner.
          </p>
          ...
          {step === 'email' && (
            <form className={styles.form} onSubmit={startSearch}>
              <h2 className={styles.sectionTitle}>Enter the Email Address</h2>
              <p className={styles.helperText}>We&apos;ll identify the owner, address, and associated records.</p>

              <div className={styles.fieldGroup}>
                <label className={styles.label} htmlFor="v2-email">Email Address</label>
                <input
                  id="v2-email"
                  type="email"
          ...
          {step === 'searching-one' && (
            <div className={styles.searching}>
              ...
              <p className={styles.helperText}>Searching email owner records...</p>
              <ul className={styles.searchList}>
                <li>Email owner records</li>
          ...
          {step === 'context' && (
            <div className={styles.form}>
              <h2 className={styles.sectionTitle}>Where Do You Think This Person Is From?</h2>
              <p className={styles.helperText}>Narrowing by state helps find results faster. Optional — skip to search all states.</p>

              <div className={styles.fieldGroup}>
                <label className={styles.label} htmlFor="v2-state">State (optional)</label>
                <select
                  id="v2-state"
...
export default EmailSearchLandingV2Page;

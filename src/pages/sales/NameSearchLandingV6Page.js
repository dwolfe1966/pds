...
const NameSearchLandingV6Page = () => {
  ...
  useEffect(() => {
    let timer;
    if (step === 'searching-one') timer = setTimeout(() => { track('search_step', { step: 'location', search_type: 'name', variant: 'v6' }); setStep('location'); }, 1700);
    if (step === 'searching-two') timer = setTimeout(() => { track('search_step', { step: 'details', search_type: 'name', variant: 'v6' }); setStep('details'); }, 1700);
  ...
  const runSearch = () => {
    ...
    // call, then redirect to ?error=true). Handing off keeps the divorce-themed entry
  ...
  const continueFromDetails = () => { track('search_step', { step: 'confirm', search_type: 'name', variant: 'v6' }); setStep('confirm'); };

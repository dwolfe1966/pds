...
const TRUST_BADGES = [
  { label: 'Background Check', icon: '🛡️' },
  { label: 'Identity Verify', icon: '🔍' },
  { label: '12B+ Records', icon: '📋' },
  { label: 'Safe Dating', icon: '💚' },
];

const BENEFIT_BULLETS = [
  'Verify their real identity',
  'Check criminal history',
  'Find their real name & address',
  'Date safely with confidence',
];

const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

/**
 * Email search landing v6 – Dating Safety Email Check.
 * 3-step flow: Email → Context/Location → Confirm → Results.
 */
const EmailSearchLandingV6Page = () => {
  const brand = useBrand();
  useLandingTrack('email', 'v6');
  ...
  useEffect(() => {
    ...
    if (step === 'searching-one') {
      timer = setTimeout(() => { track('search_step', { step: 'context', search_type: 'email', variant: 'v6' }); setStep('context'); }, 1700);
    }
    return () => {
  ...
  useEffect(() => {
    ...
    const runSearch = async () => {
      try {
        setFinalStatus('Running background check...');
        setFinalProgress(10);
  ...
  const startSearch = (e) => {
    ...
    track('search_step', { step: 'searching-one', search_type: 'email', variant: 'v6' });
    setStep('searching-one');
  };

  const continueFromContext = () => { track('search_step', { step: 'confirm', search_type: 'email', variant: 'v6' }); setStep('confirm'); };
  ...
  const handleConfirm = () => {
    ...
    track('search_step', { step: 'final-search', search_type: 'email', variant: 'v6' });
    track('fcra_agree', { search_type: 'email', variant: 'v6' });
    ...
    <main className={styles.main}>
      <div className={styles.wrapper}>
        <p className={styles.pageSubtitle}>
          <span className={styles.pageSubtitleIcon} aria-hidden>🛡️</span> Dating Safety Email Check
        </p>
        ...
        <div className={styles.card}>
          <h1 className={styles.title}>Verify Before You Trust</h1>
          <p className={styles.subtitle}>
            Enter their email to run a discreet background check before you meet in person.
          </p>
          ...
          {step === 'email' && (
            <form className={styles.form} onSubmit={startSearch}>
              <h2 className={styles.sectionTitle}>Enter Their Email Address</h2>
              <p className={styles.helperText}>We&apos;ll run a discreet background check and identity verification.</p>

              <div className={styles.fieldGroup}>
                <label className={styles.label} htmlFor="v6-email">Email Address</label>
                <input
                  id="v6-email"
                  type="email"
          ...
          {step === 'searching-one' && (
            <div className={styles.searching}>
              ...
              <p className={styles.helperText}>Running background check...</p>
          ...
          {step === 'context' && (
            <div className={styles.form}>
              <h2 className={styles.sectionTitle}>Which State Are They In?</h2>
              <p className={styles.helperText}>Optional — helps narrow criminal record search.</p>
...
export default EmailSearchLandingV6Page;

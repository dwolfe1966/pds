...
const TRUST_BADGES = [
  { label: 'People Finder', icon: '👥' },
  { label: 'Contact Records', icon: '✉️' },
...
/**
 * Email search landing v4 – Find a Lost Contact.
...
const EmailSearchLandingV4Page = () => {
  ...
  useEffect(() => {
    ...
    if (step === 'searching-one') {
      timer = setTimeout(() => { track('search_step', { step: 'context', search_type: 'email', variant: 'v4' }); setStep('context'); }, 1700);
    }
    return () => {
  ...
  useEffect(() => {
    ...
    const runSearch = async () => {
      try {
        setFinalStatus('Searching people records...');
        setFinalProgress(10);
  ...
  const startSearch = (e) => {
    ...
    track('search_step', { step: 'searching-one', search_type: 'email', variant: 'v4' });
    setStep('searching-one');
  };

  const continueFromContext = () => { track('search_step', { step: 'confirm', search_type: 'email', variant: 'v4' }); setStep('confirm'); };
  ...
  const handleConfirm = () => {
    ...
    track('search_step', { step: 'final-search', search_type: 'email', variant: 'v4' });
    track('fcra_agree', { search_type: 'email', variant: 'v4' });
    ...
    <main className={styles.main}>
      <div className={styles.wrapper}>
        <p className={styles.pageSubtitle}>
          <span className={styles.pageSubtitleIcon} aria-hidden>👥</span> People Finder
        </p>
        ...
        <div className={styles.card}>
          <h1 className={styles.title}>Reconnect with Someone You&apos;ve Lost</h1>
          <p className={styles.subtitle}>
            Have their old email? We&apos;ll help you find their current phone number and address.
          </p>
          ...
          {step === 'email' && (
            <form className={styles.form} onSubmit={startSearch}>
              ...
              <p className={styles.helperText}>We&apos;ll search people records to find their current contact info.</p>

              <div className={styles.fieldGroup}>
                <label className={styles.label} htmlFor="v4-email">Email Address</label>
                <input
                  id="v4-email"
                  type="email"
          ...
          {step === 'searching-one' && (
            <div className={styles.searching}>
              ...
              <p className={styles.helperText}>Searching people records...</p>
          ...
          {step === 'context' && (
            <div className={styles.form}>
              <h2 className={styles.sectionTitle}>Which State Were They Last In?</h2>
              <p className={styles.helperText}>Optional — helps narrow results.</p>
...
export default EmailSearchLandingV4Page;

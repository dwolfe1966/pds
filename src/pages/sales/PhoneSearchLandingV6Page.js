...
/**
 * Phone search landing v6 – Dating Safety / Verify Before You Meet.
 * 3-step flow: Phone → Location → Confirm → Results.
 */
const PhoneSearchLandingV6Page = () => {
  const brand = useBrand();
  useLandingTrack('phone', 'v6');
  ...
  useEffect(() => {
    ...
    if (step === 'searching-one') {
      timer = setTimeout(() => { track('search_step', { step: 'location', search_type: 'phone', variant: 'v6' }); setStep('location'); }, 1700);
    }
    return () => {
  ...
  const startSearch = (e) => {
    ...
    track('search_step', { step: 'searching-one', search_type: 'phone', variant: 'v6' });
    setStep('searching-one');
  };

  const continueFromLocation = () => { track('search_step', { step: 'confirm', search_type: 'phone', variant: 'v6' }); setStep('confirm'); };
  ...
  const handleConfirm = () => {
    ...
    track('search_step', { step: 'final-search', search_type: 'phone', variant: 'v6' });
    track('fcra_agree', { search_type: 'phone', variant: 'v6' });
    ...
    <main className={styles.main}>
      <div className={styles.wrapper}>
        <p className={styles.pageSubtitle}>
          <span className={styles.pageSubtitleIcon} aria-hidden>🛡️</span> Dating Safety Check
        </p>
        ...
        <div className={styles.card}>
          <h1 className={styles.title}>Verify Before You Meet</h1>
          <p className={styles.subtitle}>
            Enter their phone number to run a discreet background check before your first date.
          </p>
          ...
          {step === 'phone' && (
            <form className={styles.form} onSubmit={startSearch}>
              <h2 className={styles.sectionTitle}>Enter Their Phone Number</h2>
              <p className={styles.helperText}>We&apos;ll run a discreet background check and identity verification.</p>

              <div className={styles.fieldGroup}>
                <label className={styles.label} htmlFor="v6-phone">Phone Number</label>
                <input
                  id="v6-phone"
                  type="tel"
...
export default PhoneSearchLandingV6Page;

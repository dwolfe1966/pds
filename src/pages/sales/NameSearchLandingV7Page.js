 ...
 * DESIGN: Spokeo-style palette (blue #007cc2 + high-contrast orange CTA #fd6f0b),
 * a single search-bar hero (first+last styled as one unit) and a big-number proof
 * band — distinct color system + IA from the green v2–v6.
 * FLOW/TRACKING: identical wizard + search_step/validation_error/fcra_agree events
 * as v3 (variant tagged 'v7'). Same Name → Location → Details → Confirm → loader.
 */
...
const NameSearchLandingV7Page = () => {
  const brand = useBrand();
  useLandingTrack('name', 'v7');
  ...
  useEffect(() => {
    let t;
    if (step === 'searching-one') t = setTimeout(() => { track('search_step', { step: 'location', search_type: 'name', variant: 'v7' }); setStep('location'); }, 1700);
    if (step === 'searching-two') t = setTimeout(() => { track('search_step', { step: 'details', search_type: 'name', variant: 'v7' }); setStep('details'); }, 1700);
  ...
  const startSearch = (e) => {
    ...
    track('search_step', { step: 'searching-one', search_type: 'name', variant: 'v7' }); setStep('searching-one');
  };
  const continueFromLocation = () => {
    ...
    setLocationError(''); track('search_step', { step: 'searching-two', search_type: 'name', variant: 'v7' }); setStep('searching-two');
  };
  const continueFromDetails = () => { track('search_step', { step: 'confirm', search_type: 'name', variant: 'v7' }); setStep('confirm'); };
  const handleConfirm = () => {
    setAgreeError('');
    ...
    track('search_step', { step: 'final-search', search_type: 'name', variant: 'v7' });
    track('fcra_agree', { search_type: 'name', variant: 'v7' });
  ...
  return (
    <main style={{ minHeight: '100vh', background: `linear-gradient(180deg, ${P.bg} 0%, #ffffff 45%)`, fontFamily: 'inherit' }}>
      {/* Blue hero band */}
      ...
      <div style={{ maxWidth: 540, margin: '-2.5rem auto 0', padding: '0 1rem 3rem' }}>
        <div style={card}>
          ...
          {/* Step 1 — single search-bar hero */}
          {step === 'name' && (
            <form onSubmit={startSearch}>
              <h2 style={{ margin: '0 0 0.2rem', fontSize: '1.15rem', fontWeight: 800, color: P.ink }}>Who are you looking for?</h2>
              <p style={{ margin: '0 0 1rem', fontSize: '0.88rem', color: P.mut }}>Enter a first and last name to search billions of public records.</p>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-around', textAlign: 'center', marginTop: '1.25rem', paddingTop: '1.1rem', borderTop: `1px solid ${P.line}` }}>
                {[['Billions', 'Records'], ['120+', 'Data sources'], ['50', 'States']].map(([n, l]) => (
          ...
          {step === 'searching-one' && <Searching title="Searching public records…" P={P} items={['Public records', 'Contact databases', 'Court &amp; property records']} />}

          {/* Step 2 — Location */}
          {step === 'location' && (
            <div>
              <h2 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', fontWeight: 800, color: P.ink }}>Which state?</h2>
              <p style={{ margin: '0 0 1rem', fontSize: '0.86rem', color: P.mut }}>Narrows results to the right person. City is optional.</p>
              <label style={label} htmlFor="v7-city">City (optional)</label>
              <input id="v7-city" style={{ ...input, marginBottom: '0.75rem' }} value={city} onChange={(e) => setCity(e.target.value)} placeholder="City" />
              <label style={label} htmlFor="v7-state">State *</label>
              <select id="v7-state" style={{ ...input, marginBottom: '1rem', borderColor: locationError ? '#b91c1c' : P.line }} value={state} onChange={(e) => { setState(e.target.value); if (locationError) setLocationError(''); }}>
          ...
          {step === 'searching-two' && <Searching title={`Finding matches for ${firstName} ${lastName}…`} P={P} items={['Matching identities', 'Checking records', 'Cross-referencing sources']} />}

          {/* Step 3 — Details */}
          {step === 'details' && (
            <div>
              <h2 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', fontWeight: 800, color: P.ink }}>Possible matches found</h2>
              <p style={{ margin: '0 0 1rem', fontSize: '0.86rem', color: P.mut }}>Add age or middle name to narrow results. Optional.</p>
              <label style={label} htmlFor="v7-age">Age (optional)</label>
              <input id="v7-age" style={{ ...input, marginBottom: '0.75rem' }} value={age} onChange={(e) => setAge(e.target.value)} placeholder="Age" inputMode="numeric" />
              <label style={label} htmlFor="v7-mid">Middle name (optional)</label>
              <input id="v7-mid" style={{ ...input, marginBottom: '1rem' }} value={middleName} onChange={(e) => setMiddleName(e.target.value)} placeholder="Middle name" />
          ...
          {step === 'confirm' && (
            <div>
              <h2 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', fontWeight: 800, color: P.ink }}>Confirm to view results</h2>
...
export default NameSearchLandingV7Page;

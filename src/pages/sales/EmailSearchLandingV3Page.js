...
const EmailSearchLandingV3Page = () => {
  ...
  useEffect(() => {
    ...
    if (step === 'searching-one') {
      timer = setTimeout(() => { track('search_step', { step: 'context', search_type: 'email', variant: 'v3' }); setStep('context'); }, 1700);
    }
    return () => {
  ...
  const startSearch = (e) => {
    ...
    track('search_step', { step: 'searching-one', search_type: 'email', variant: 'v3' });
    setStep('searching-one');
  };

  const continueFromContext = () => { track('search_step', { step: 'confirm', search_type: 'email', variant: 'v3' }); setStep('confirm'); };
  ...
  const handleConfirm = () => {
    ...
    track('search_step', { step: 'final-search', search_type: 'email', variant: 'v3' });
    track('fcra_agree', { search_type: 'email', variant: 'v3' });

...
/**
 * Name landing v8 — "Dark premium" design exploration.
 * DESIGN: charcoal (#16213e) + amber CTA (#f59e0b), a SPLIT layout (wizard card left, a
 * "what every report includes" preview panel right — stacks on mobile). Distinct color +
 * IA from the green v2–v6 and the blue v7. Self-chrome (global green nav/footer suppressed).
 * FLOW/TRACKING: identical wizard + search_step/validation_error/fcra_agree (variant 'v8').
 */
...
const NameSearchLandingV8Page = () => {
  const brand = useBrand();
  useLandingTrack('name', 'v8');
  ...
  useEffect(() => {
    let t;
    if (step === 'searching-one') t = setTimeout(() => { track('search_step', { step: 'location', search_type: 'name', variant: 'v8' }); setStep('location'); }, 1700);
    if (step === 'searching-two') t = setTimeout(() => { track('search_step', { step: 'details', search_type: 'name', variant: 'v8' }); setStep('details'); }, 1700);
  ...
  const startSearch = (e) => {
    ...
    track('search_step', { step: 'searching-one', search_type: 'name', variant: 'v8' }); setStep('searching-one');
  };
  const continueFromLocation = () => {
    ...
    setLocationError(''); track('search_step', { step: 'searching-two', search_type: 'name', variant: 'v8' }); setStep('searching-two');
  };
  const continueFromDetails = () => { track('search_step', { step: 'confirm', search_type: 'name', variant: 'v8' }); setStep('confirm'); };
  const handleConfirm = () => {
    setAgreeError('');
    ...
    track('search_step', { step: 'final-search', search_type: 'name', variant: 'v8' });
    track('fcra_agree', { search_type: 'name', variant: 'v8' });
  ...
  return (
    <main style={{ minHeight: '100vh', background: `linear-gradient(180deg, ${P.bg0} 0%, ${P.bg1} 100%)`, color: P.ink }}>
      ...
      <div style={{ padding: '1.6rem 1.1rem 0.5rem', maxWidth: 920, margin: '0 auto', textAlign: 'center' }}>
        <p style={{ margin: 0, fontSize: '0.78rem', fontWeight: 800, letterSpacing: '0.08em', color: P.amber }}>◆ {brand.name.toUpperCase()}</p>
        <h1 style={{ margin: '0.4rem 0 0.3rem', fontSize: '1.85rem', fontWeight: 800, lineHeight: 1.1, color: P.ink }}>The complete background report.</h1>
        <p style={{ margin: 0, fontSize: '0.95rem', color: P.mut, maxWidth: 460, marginInline: 'auto' }}>Contact info, addresses, relatives, and court records — in one search.</p>
      </div>

      {/* Split: form + preview panel */}
      <div style={{ maxWidth: 920, margin: '1rem auto 0', padding: '0 1rem 2.5rem', display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'flex-start' }}>
        ...
        <div style={{ ...cardStyle, flex: '1 1 320px' }}>
          ...
          {step === 'name' && (
            <form onSubmit={startSearch}>
              <h2 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', fontWeight: 800, color: P.ink }}>Run a search</h2>
              <p style={{ margin: '0 0 1rem', fontSize: '0.86rem', color: P.mut }}>Enter a first and last name to begin.</p>
          ...
          {step === 'location' && (
            <div>
              ...
              <p style={{ margin: '0 0 1rem', fontSize: '0.85rem', color: P.mut }}>Narrows to the right person. City optional.</p>
          ...
          {step === 'details' && (
            <div>
              <h2 style={{ margin: '0 0 0.2rem', fontSize: '1.05rem', fontWeight: 800, color: P.ink }}>Possible matches found</h2>
          ...
          {step === 'confirm' && (
            <div>
              <h2 style={{ margin: '0 0 0.2rem', fontSize: '1.05rem', fontWeight: 800, color: P.ink }}>Confirm to view results</h2>
        ...
        {/* Right: "what's in every report" preview panel */}
        <div style={{ ...cardStyle, flex: '1 1 240px', background: P.panel }}>
          <p style={{ margin: '0 0 0.75rem', fontSize: '0.78rem', fontWeight: 800, letterSpacing: '0.04em', color: P.amber }}>EVERY REPORT INCLUDES</p>
          {[['📞', 'Phone numbers'], ['✉️', 'Email addresses'], ['🏠', 'Address history'], ['👥', 'Relatives & associates'], ['⚠️', 'Court & criminal records'], ['💼', 'Work & education']].map(([icon, l]) => (
          ...
          <p style={{ margin: '0.75rem 0 0', fontSize: '0.72rem', color: P.mut }}>Sourced from billions of public records across 50 states.</p>
        </div>
      </div>
...
export default NameSearchLandingV8Page;

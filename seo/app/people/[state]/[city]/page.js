  ...
  return (
    <main style={ui.main}>
      ...
      <section style={ui.hero}>
        <p style={ui.eyebrow}>City directory</p>
        ...
        <a href={`${MAIN}/name/landing/v2?utm_source=idlookup.me&utm_medium=referral&utm_campaign=people-directory&state=${c.stateCode}`}
          style={{ ...ui.cta, fontSize: 16, padding: '14px 22px' }}>
      ...
      {stats.length > 0 && (
        <section style={ui.card}>
          ...
          {prose && <p style={{ margin: '14px 0 0', fontSize: 14, lineHeight: 1.65, color: ui.color.body }}>{prose}</p>}
      ...
      {names.length > 0 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Most common names in {c.city}</h2>
      ...
      {stateNames.length > 0 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Popular names in {c.stateName}</h2>
          <p style={{ ...ui.muted, margin: '0 0 12px', fontSize: 13, lineHeight: 1.6 }}>
      ...
      {occupations.length > 0 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Occupations in {c.city}</h2>
          {occupations.map((o) => (
            <div key={o.label} style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: ui.color.body, marginBottom: 2 }}>
                <span>{o.label}</span><span style={{ color: ui.color.muted, fontWeight: 700 }}>{o.value}%</span>
              </div>
              <span style={{ display: 'block', height: 8, background: '#eef2f0', borderRadius: 999, overflow: 'hidden' }}>
                <span style={{ display: 'block', height: '100%', background: ui.color.accent, width: `${Math.min(100, o.value)}%` }} />
              </span>
            </div>
          ))}
          <p style={ui.source}>Share of employed residents (16+). Source: U.S. Census Bureau, ACS (5-year).</p>
        </section>
      )}

      {popPoints.length >= 4 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Population trend</h2>
      ...
      {stateCities.length >= 3 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Where {c.city} is</h2>
          <StateMap cities={stateCities} name={c.stateName} highlight={c.city} width={680} height={380} />
          <p style={ui.source}>{c.city} (highlighted) among major cities in {c.stateName}.</p>
        </section>
      )}

      {people && people.length > 0 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Notable people from {c.city}</h2>
          <div style={ui.linkGrid}>
            {people.slice(0, 10).map((p) => (
              <a key={p.url} href={p.url} target="_blank" rel="noopener" style={{ ...ui.link, fontSize: 14 }}>{p.name}</a>
            ))}
          </div>
          <p style={ui.source}>Born in {c.city}. Source: Wikidata / Wikipedia (CC0); links open Wikipedia.</p>
        </section>
      )}

      {historic && historic.count > 0 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Historic places in {c.city}</h2>
          ...
          <div style={ui.linkGrid}>
            {historic.places.map((p, i) => {
              ...
              return p.url
                ...
                : <span key={i} style={{ fontSize: 14, color: ui.color.body }}>{p.name}{star}</span>;
            })}
          </div>
          <p style={ui.source}>★ = National Historic Landmark. Source: NPS National Register of Historic Places.</p>
        </section>
      )}

      {newspapers && newspapers.length > 0 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Historic newspapers of {c.city}</h2>
      ...
      {nearby.length > 0 && (
        <section style={ui.card}>
          <h2 style={ui.h2}>Cities near {c.city}</h2>
          <div style={ui.linkGrid}>
            {nearby.map((n) => (
      ...
      <a href={`${MAIN}/name/landing/v2?utm_source=idlookup.me&utm_medium=referral&utm_campaign=people-directory&state=${c.stateCode}`} style={ui.secondaryCta}>Search people in {c.city}</a>
